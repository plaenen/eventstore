...
│   │   ├── *_aggregate.es.pb.go    # Generated event sourcing code (DO NOT EDIT)
│   │   ├── *_client.es.pb.go       # Generated client SDK (DO NOT EDIT)
│   │   ├── *_handler.es.pb.go      # Generated handler interfaces (DO NOT EDIT)
│   │   ├── *_sdk.es.pb.go          # Generated SDK (DO NOT EDIT)
│   │   └── *_server.es.pb.go       # Generated server (DO NOT EDIT)
│   └── README.md
│
├── bankaccount/
│   ├── domain/              # ✍️  Domain logic
│   │   └── account_appliers.go  # Event applier implementations
│   └── handlers/            # ✍️  Application logic
│       ├── command_handler.go   # Business logic for commands
│       └── query_handler.go     # Business logic for queries
│
├── cmd/                    # Runnable demo applications
│   ├── bankaccount-observability/   # SQLite + observability demo
│   ├── generic-projection/          # Cross-domain projection demo
│   ├── projection-migrations/       # Migration-based projection demo
│   ├── projection-nats/             # NATS event bus demo
│   └── sqlite-projection/           # Basic SQLite projection demo
│
└── sdk/                    # Generated unified SDK
    └── unified.go          # Combines all service SDKs
```
...
|---------|------|-----------|
| `*.pb.go` | Generated by protoc-gen-go | ❌ Never edit |
| `*.connect.go` | Generated by protoc-gen-connect | ❌ Never edit |
| `*.es.pb.go` | Generated by protoc-gen-eventsourcing | ❌ Never edit |
| `*.go` (no suffix) | Hand-written by developers | ✅ Maintain these |

**Rule: Files ending in `.pb.go`, `.es.pb.go`, or `.connect.go` are generated. Plain `.go` files are yours.**

The `.es.pb.go` extension clearly identifies eventsourcing-generated files (similar to how gRPC uses `_grpc.pb.go`).
...
The `cmd/` directory contains runnable demo applications showcasing different features:

### Bank Account + Observability
```bash
go run ./examples/cmd/bankaccount-observability
```
Demonstrates SQLite event store with OpenTelemetry observability (traces and metrics).

### Generic Projection
```bash
go run ./examples/cmd/generic-projection
```
Shows how to build projections that combine events from multiple aggregates/domains.

### Projection with Migrations
```bash
go run ./examples/cmd/projection-migrations
```
Demonstrates version-controlled schema evolution using embedded SQL migrations.

### SQLite Projection
```bash
go run ./examples/cmd/sqlite-projection
```
Basic example of building a SQLite projection with automatic transaction management.

### NATS Projection (WIP)
```bash
go run ./examples/cmd/projection-nats
```
Shows integration with NATS JetStream for real-time event processing.
...
This regenerates all `.pb.go`, `.es.pb.go`, and `.connect.go` files.
