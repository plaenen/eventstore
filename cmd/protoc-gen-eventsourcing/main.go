...
func generateProjections(g *protogen.GeneratedFile, file *protogen.File, projections []*ProjectionInfo) {
	for _, proj := range projections {
		...
		g.P("	return eventsourcing.NewProjection(b.name, b.handlers, b.resetFunc)")
		g.P("}")
		g.P()
	}
}
...
func generateSQLiteProjections(g *protogen.GeneratedFile, file *protogen.File, projections []*ProjectionInfo) {
	for _, proj := range projections {
		...
		g.P("func (b *", builderType, ") WithMigrations(migrationsFS interface{}, path string) *", builderType, " {")
		g.P("	// We use interface{} for fs to avoid importing io/fs if not needed, but sqlite builder expects fs.FS")
		g.P("	// Let's assume user passes fs.FS")
		g.P("	if fs, ok := migrationsFS.(interface{ Open(string) (interface{}, error) }); ok {")
		g.P("		b.builder.WithMigrations(fs, path)")
		g.P("	}")
		g.P("	return b")
		g.P("}")
		g.P()
