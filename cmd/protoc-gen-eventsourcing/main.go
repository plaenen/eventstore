...
// generateAggregateHandler generates a single handler interface for an aggregate
// combining all methods from all services that operate on this aggregate
func generateAggregateHandler(g *protogen.GeneratedFile, aggregateName string, services []*ServiceInfo) {
	handlerName := aggregateName + "Handler"
	unimplementedName := "Unimplemented" + aggregateName + "Handler"

	// Generate main handler interface
	g.P("// ", handlerName, " defines methods for ", aggregateName, " aggregate operations")
	g.P("// Each method receives the request and functional options for context like authentication, tracing, etc.")
	g.P("type ", handlerName, " interface {")

	// Collect all methods from all services for this aggregate
	for _, svc := range services {
		// Generate methods for commands
		for _, method := range svc.Commands {
			methodName := method.GoName
			inputType := g.QualifiedGoIdent(method.Input.GoIdent)
			outputType := g.QualifiedGoIdent(method.Output.GoIdent)

			g.P("	// ", methodName, " handles the ", methodName, " operation")
			g.P("	", methodName, "(ctx context.Context, req *", inputType, ", opts ...eventsourcing.MethodOption) (*", outputType, ", error)")
		}

		// Generate methods for queries
		for _, method := range svc.Queries {
			methodName := method.GoName
			inputType := g.QualifiedGoIdent(method.Input.GoIdent)
			outputType := g.QualifiedGoIdent(method.Output.GoIdent)
	...
	// Generate Unimplemented base struct
	g.P("// ", unimplementedName, " provides default implementations returning 'not implemented' errors")
	g.P("// Embed this in your implementation to only implement the methods you need:")
	g.P("//")
	g.P("//   type MyHandler struct {")
	g.P("//       ", unimplementedName)
	g.P("//       // your fields...")
	g.P("//   }")
	g.P("//")

	// Find first method for example
	var firstMethod *protogen.Method
	var firstInput, firstOutput protogen.GoIdent
	for _, svc := range services {
		if len(svc.Commands) > 0 {
			firstMethod = svc.Commands[0]
			firstInput = firstMethod.Input.GoIdent
			firstOutput = firstMethod.Output.GoIdent
			break
		} else if len(svc.Queries) > 0 {
			firstMethod = svc.Queries[0]
			firstInput = firstMethod.Input.GoIdent
			firstOutput = firstMethod.Output.GoIdent
			break
		}
	...
	if firstMethod != nil {
		g.P("//   func (h *MyHandler) ", firstMethod.GoName, "(ctx context.Context, req *", g.QualifiedGoIdent(firstInput), ", opts ...eventsourcing.MethodOption) (*", g.QualifiedGoIdent(firstOutput), ", error) {")
		g.P("//       // Your implementation")
		g.P("//   }")
	}

	g.P("type ", unimplementedName, " struct{}")
	g.P()

	// Generate unimplemented methods for all services
	for _, svc := range services {
		// Commands
		for _, method := range svc.Commands {
			methodName := method.GoName
			...
			g.P("// ", methodName, " returns a 'not implemented' error")
			g.P("func (h *", unimplementedName, ") ", methodName, "(ctx context.Context, req *", inputType, ", opts ...eventsourcing.MethodOption) (*", outputType, ", error) {")
			g.P(`	return nil, &eventsourcing.NotImplementedError{Method: "`, methodName, `"}`)
			g.P("}")
			g.P()
		}
...
func generateHandlerInterfaces(gen *protogen.Plugin, file *protogen.File, aggregates []*AggregateInfo, services []*ServiceInfo) {
	...
	g.P("// Code generated by protoc-gen-eventsourcing. DO NOT EDIT.")
	g.P("// version: ", version)
	g.P("// Aggregate handler interfaces for ", file.Desc.Name())
	g.P()
	g.P("package ", file.GoPackageName)
	g.P()
	g.P("import (")
	...
	for _, svc := range services {
		...
		if opts != nil && opts.GetAggregateHandler() {
			...
			legacyServices = append(legacyServices, svc)
		}
	}
	...
	// Generate legacy handlers (one interface per service using AppError)
...
func generateServerService(gen *protogen.Plugin, file *protogen.File, aggregates []*AggregateInfo, services []*ServiceInfo) {
	...
	for _, svc := range services {
		opts := getServiceOptions(svc.Service)
		useAggregateHandler := opts != nil && opts.GetAggregateHandler()

		serverName := svc.Name + "Server"
		var handlerName string

		if useAggregateHandler {
			handlerName = opts.GetAggregateName() + "Handler"
		} else {
			handlerName = svc.Name + "Handler"
		}
