...
func main() {
	if len(os.Args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: %s <pb-directory> <output-file>\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "Example: %s ./examples/pb ./examples/sdk/unified.go\n", os.Args[0])
		os.Exit(1)
	}
	...
	services, err := discoverServices(pbDir)
	if err != nil {
	...
	if err := generateUnifiedSDK(services, outputFile); err != nil {
...
func discoverServices(pbDir string) ([]ServiceSDK, error) {
	...
	err := filepath.Walk(pbDir, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() && strings.HasSuffix(info.Name(), "_sdk.pb.go") {
			sdks, err := parseSDKFile(path, pbDir)
			if err != nil {
...
func parseSDKFile(filePath, pbDir string) ([]ServiceSDK, error) {
...
func generateUnifiedSDK(services []ServiceSDK, outputFile string) error {
