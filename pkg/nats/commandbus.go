...
func NewCommandBus(config CommandBusConfig) (*CommandBus, error) {
	...
	return &CommandBus{
		...
		middleware: make([]eventsourcing.CommandMiddleware, 0),
...
func (b *CommandBus) Register(commandType string, handler eventsourcing.CommandHandler) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *CommandBus) Use(middleware eventsourcing.CommandMiddleware) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *CommandBus) Send(ctx context.Context, cmd *eventsourcing.CommandEnvelope) error {
	if cmd == nil {
		return eventsourcing.ErrInvalidCommand
	}

	// Get command type
	commandType := cmd.Metadata.Custom["command_type"]
	if commandType == "" {
		return fmt.Errorf("%w: command_type not specified in metadata", eventsourcing.ErrInvalidCommand)
	}
...
func (b *CommandBus) serializeCommandEnvelope(cmd *eventsourcing.CommandEnvelope) ([]byte, error) {
...
func (b *CommandBus) deserializeCommandEnvelope(data []byte) (*eventsourcing.CommandEnvelope, error) {
	...
	return &eventsourcing.CommandEnvelope{
...
func (b *CommandBus) sendSuccessResponse(msg *nats.Msg, events []*eventsourcing.Event) {
