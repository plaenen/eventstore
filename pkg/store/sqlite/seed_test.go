...
func TestSeedEvents_Basic(t *testing.T) {
	...
	result, err := store.SeedEvents(aggregateID, 0, events, domain.DefaultSeedOptions())
	require.NoError(t, err)
...
func TestSeedEvents_Idempotency(t *testing.T) {
	...
	// First seed
	result1, err := store.SeedEvents(aggregateID, 0, events, domain.DefaultSeedOptions())
	require.NoError(t, err)
	assert.Equal(t, 1, result1.Saved)
	assert.Equal(t, 0, result1.Skipped)

	// Second seed (should skip)
	result2, err := store.SeedEvents(aggregateID, 0, events, domain.DefaultSeedOptions())
	require.NoError(t, err)
...
func TestSeedEvents_WithConstraints_OwnershipCheck(t *testing.T) {
	...
	result1, err := store.SeedEvents(aggregateID1, 0, events1, domain.DefaultSeedOptions())
	require.NoError(t, err)
	assert.Equal(t, 1, result1.Saved)
	...
	result2, err := store.SeedEvents(aggregateID2, 0, events2, domain.DefaultSeedOptions())
	require.NoError(t, err)
	assert.Equal(t, 0, result2.Saved)
	...
	result3, err := store.SeedEvents(aggregateID1, 0, events1, domain.DefaultSeedOptions())
	require.NoError(t, err)
...
func TestSeedEvents_EmptyEventList(t *testing.T) {
	...
	result, err := store.SeedEvents("test-id", 0, []*domain.Event{}, nil)
	require.NoError(t, err)
...
func TestSeedEvents_PartialFailure(t *testing.T) {
	...
	_, err = store.SeedEvents(aggregateID1, 0, firstEvent, domain.DefaultSeedOptions())
	require.NoError(t, err)
	...
	result, err := store.SeedEvents(aggregateID2, 0, events, domain.DefaultSeedOptions())
	require.NoError(t, err)
