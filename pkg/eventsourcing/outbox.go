...
// defaultOutboxLogger provides a simple log-based implementation.
type defaultOutboxLogger struct{}

func (l *defaultOutboxLogger) Info(msg string, args ...interface{}) {
	log.Printf("[INFO] "+msg, args...)
}

func (l *defaultOutboxLogger) Error(msg string, args ...interface{}) {
	log.Printf("[ERROR] "+msg, args...)
}

func (l *defaultOutboxLogger) Warn(msg string, args ...interface{}) {
...
func DefaultOutboxForwarderConfig() OutboxForwarderConfig {
	return OutboxForwarderConfig{
		...
		BatchSize:  100,
		MaxRetries: 10,
		Logger:     &defaultOutboxLogger{},
	}
}

// NewOutboxForwarder creates a new outbox forwarder with the given configuration.
func NewOutboxForwarder(store EventStore, bus EventBus, cfg OutboxForwarderConfig) *OutboxForwarder {
	if cfg.PollRate == 0 {
	...
	if cfg.Logger == nil {
		cfg.Logger = &defaultOutboxLogger{}
	}
...
func (f *OutboxForwarder) processBatch(ctx context.Context) error {
	...
	// Extract events from envelopes (EventBus.Publish expects []*Event)
	events := make([]*Event, len(envelopes))
