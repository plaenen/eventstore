...
func TestSnapshotMetadata_AnalyticsRoundTrip(t *testing.T) {
	...
	// Unmarshal metadata
	restoredMetadata, err := eventsourcing.UnmarshalMetadata(metadataJSON)
	require.NoError(t, err)
...
func TestSnapshotMetadata_AnalyticsJSONMarshaling(t *testing.T) {
	...
	// Unmarshal from JSON
	var restored eventsourcing.SnapshotMetadata
...
func TestSnapshot_FullWorkflowWithAnalytics(t *testing.T) {
	...
	agg := eventsourcing.NewAggregateRoot("order-456", "Order")
	events := []*eventsourcing.Event{
		{
			ID:            "evt-1",
	...
	newAgg := eventsourcing.NewAggregateRoot(snapshot.AggregateID, snapshot.AggregateType)

	// Parse analytics back to EventAnalytics struct
	analyticsJSON, err := json.Marshal(restoredAnalytics)
	require.NoError(t, err)

	var parsedAnalytics eventsourcing.EventAnalytics
