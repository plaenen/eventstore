...
type TransactionalEventHandler func(ctx context.Context, tx *sql.Tx, envelope *eventsourcing.EventEnvelope) error
...
// system as the event eventsourcing.
//
...
//	    return err
//	}))
func (b *SQLiteProjectionBuilder) On(registration eventsourcing.EventHandlerRegistration) *SQLiteProjectionBuilder {
	// Wrap the handler to inject transaction
	b.handlers[registration.EventType] = func(ctx context.Context, tx *sql.Tx, envelope *eventsourcing.EventEnvelope) error {
...
func (b *SQLiteProjectionBuilder) Build() (eventsourcing.Projection, error) {
	...
	_ = b.statusStore.Save(&eventsourcing.ProjectionState{
...
// SQLiteProjection implements eventsourcing.Projection with SQLite-specific features.
...
func (p *SQLiteProjection) Handle(ctx context.Context, envelope *eventsourcing.EventEnvelope) error {
...
func (p *SQLiteProjection) Rebuild(ctx context.Context) error {
	...
	rebuildState := &eventsourcing.ProjectionState{
		ProjectionName: p.name,
		Status:         eventsourcing.ProjectionStatusRebuilding,
		Message:        "Starting rebuild from event store",
		UpdatedAt:      eventsourcing.Now(),
		Progress: &eventsourcing.RebuildProgress{
			EventsProcessed: 0,
			TotalEvents:     0, // Unknown
			StartedAt:       eventsourcing.Now(),
		},
	}
	...
	if err := p.Reset(ctx); err != nil {
		...
		_ = p.statusStore.Save(&eventsourcing.ProjectionState{
			ProjectionName: p.name,
			Status:         eventsourcing.ProjectionStatusFailed,
	...
	for {
		...
		for _, event := range events {
			envelope := &eventsourcing.EventEnvelope{Event: *event}
			...
			if eventsProcessed%100 == 0 {
				_ = p.statusStore.UpdateProgress(p.name, &eventsourcing.RebuildProgress{
...
func (p *SQLiteProjection) GetCheckpoint(ctx context.Context) (*eventsourcing.ProjectionCheckpoint, error) {
	return p.checkpointStore.Load(p.name)
}

// GetStatus returns the current projection status.
func (p *SQLiteProjection) GetStatus(ctx context.Context) (*eventsourcing.ProjectionState, error) {
...
func (p *SQLiteProjection) IsReady(ctx context.Context) bool {
	...
	return status.Status == eventsourcing.ProjectionStatusReady
}
...
// migration system as the event eventsourcing.
//
