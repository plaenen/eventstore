...
func (s *EventStore) getCommandResultNoLock(commandID string) (*eventsourcing.CommandResult, error) {
	ctx := context.Background()
	row, err := s.queries.GetProcessedCommand(ctx, sqlcgen.GetProcessedCommandParams{
		CommandID: commandID,
		ExpiresAt: eventsourcing.Now().Unix(),
	})
...
func (s *EventStore) loadEventByID(eventID string) (*eventsourcing.Event, error) {
	ctx := context.Background()
...
func (s *EventStore) CleanExpiredCommands() (int64, error) {
	...
	ctx := context.Background()
	rowsAffected, err := s.queries.CleanExpiredCommands(ctx, eventsourcing.Now().Unix())
	if err != nil {
