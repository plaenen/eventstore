...
// ProjectionMetadataStore is a SQLite-based implementation of eventsourcing.ProjectionMetadataStore.
...
// metadataStoreConfig holds internal configuration for the metadata eventsourcing.
...
func (s *ProjectionMetadataStore) Set(projectionName, key, value string) error {
	...
	err := s.queries.SetMetadata(ctx, sqlcgen.SetMetadataParams{
		...
		Key:            key,
		Value:          value,
		UpdatedAt:      eventsourcing.Now().Unix(),
	})
