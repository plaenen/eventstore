...
import (
	...
	"github.com/plaenen/eventstore/pkg/eventsourcing/store/sqlite/sqlcgen"
...
// eventStoreConfig holds internal configuration for the SQLite event eventsourcing.
...
func validateAppendEventsInput(aggregateID string, events []*eventsourcing.Event) error {
...
// See eventsourcing.SeedOptions for configuration details.
func (s *EventStore) SeedEvents(
	aggregateID string,
	expectedVersion int64,
	events []*eventsourcing.Event,
	opts *eventsourcing.SeedOptions,
) (*eventsourcing.SeedResult, error) {
	if len(events) == 0 {
	...
	result := &eventsourcing.SeedResult{
	...
	preparedEvents := make([]*eventsourcing.Event, len(events))
	for i, event := range events {
		...
		if preparedEvent.ID == "" && opts.GenerateDeterministicIDs {
			preparedEvent.ID = eventsourcing.GenerateDeterministicSeedID(&preparedEvent)
		}

		// Augment with seed metadata
		eventsourcing.AugmentSeedMetadata(&preparedEvent, opts)
	...
	eventsToSave := make([]*eventsourcing.Event, 0, len(preparedEvents))
	...
	for _, event := range eventsToSave {
		...
		if err := s.validateSeedConstraints(tx, event, aggregateID, opts); err != nil {
			result.Failed++
			result.Errors = append(result.Errors, eventsourcing.SeedError{
				EventID:   event.ID,
				EventType: event.EventType,
				Version:   event.Version,
...
func (s *EventStore) checkExistingEventIDs(events []*eventsourcing.Event) (map[string]bool, error) {
	if len(events) == 0 {
...
func (s *EventStore) validateSeedConstraints(tx *sql.Tx, event *eventsourcing.Event, aggregateID string, opts *eventsourcing.SeedOptions) error {
...
func (s *EventStore) insertOutbox(tx *sql.Tx, event *eventsourcing.Event) error {
	_, err := tx.Exec(`
...
func (s *EventStore) LoadUnpublishedEvents(limit int) ([]*eventsourcing.EventEnvelope, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
